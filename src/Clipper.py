...
class Clipper(object):
    ...
    def simple_feature_from_masks(self, masks_np, i, j):
        
        '''
            Retrieve the indices of the masks where the value at position (i, j) is true.
            return index
        '''
        index = (masks_np[:, i.reshape(-1), j.reshape(-1)])
        return index
    ...
    def choose_pixels(self, H, W, way = 'random'):
        
        '''
            all returns the coordinates of all pixels, while random randomly samples pixels and returns their coordinates.
        '''

        if way == 'all' :
            y_coords, x_coords = np.indices((H, W))
            y_coords, x_coords = y_coords.reshape((-1)), x_coords.reshape((-1))
            num = H * W
        
        elif way == 'random':
            all_coords = np.indices((H, W), dtype=int)
            all_coords = np.stack(all_coords, axis=-1).reshape(-1, 2)
            random_sample_coords = all_coords[np.random.choice(all_coords.shape[0], size=10000, replace=False)]

            # y_coords, x_coords = np.meshgrid(random_sample_coords[:, 0], random_sample_coords[:, 1])
            y_coords, x_coords = random_sample_coords[:, 0], random_sample_coords[:, 1]
            num =  10000
