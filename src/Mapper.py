...
from src.common import (get_camera_from_tensor, get_samples_tracking_semantic, get_tensor_from_camera, random_select, get_samples, normalize_3d_coordinate, normalize_3d_coordinate_semantic)
...
class Mapper(object):
    ...
    def __init__(self, cfg, args, slam):
        ...
        self.query_text = ["chair",  "EXIT", "poster", "tap", "trash can", "white wire", "black shoes", "red book",  "window", "plotted plant"]
    ...
    def optimize_map(self, iters, lr_factor, idx, gt_color, gt_depth, gt_c2w, keyframe_dict, keyframe_list, cur_c2w, init):
        ...
        color_map = 'jet'
        semantics = (plt.cm.get_cmap(color_map)(semantic_np)[..., :3] * 255).reshape(-1, 3).reshape(H, W, 3).astype(np.uint8)
        cv2.imwrite(f'{self.output}/{self.query_text[0]}/semanticprobs/semanticprobs_{idx:04d}.png', cv2.cvtColor(semantics, cv2.COLOR_BGR2RGB))
    ...
    def build_semantic_map(self, idx, gt_color, gt_depth, gt_c2w):
        ...
        depth = depth[y_coords, x_coords].reshape((-1))
        
        y_coords, x_coords = torch.from_numpy(y_coords).to(self.device), torch.from_numpy(x_coords).to(self.device)
        x_c = (x_coords - cx) * depth / fx
        y_c = (y_coords - cy) * depth / fy
        z_c = depth
        one_c = torch.ones(z_c.shape[0], device=self.device)
        ...
        mask = (points[:, 0] < self.bound[0][1]) & (points[:, 0] > self.bound[0][0]) & \
                   (points[:, 1] < self.bound[1][1]) & (points[:, 1] > self.bound[1][0]) & \
                   (points[:, 2] < self.bound[2][1]) & (points[:, 2] > self.bound[2][0]) & \
                   ((points[:, 0] > 0) | (points[:, 0] < 0) | (points[:, 1] > 0) | (points[:, 1] < 0) | (points[:, 2] > 0) | (points[:, 2] < 0))
        ...
        with torch.no_grad():
            ...
            features[0, :, p_nor[:, 0], p_nor[:, 1], p_nor[:, 2]] = feature.T * new_w + features[0, :, p_nor[:, 0], p_nor[:, 1], p_nor[:, 2]] * (1 - new_w)
