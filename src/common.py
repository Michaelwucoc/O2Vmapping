...
def select_uv_semantic(i, j, n, depth, color, semantic, device='cuda:0'):
    """
    Select n uv from dense uv.
...
def get_sample_uv_semantic(H0, H1, W0, W1, n, depth, color, semantic, device='cuda:0'):
    """
    Sample n uv coordinates from an image region H0..H1, W0..W1
    ...
    i = i.t()  # transpose
    j = j.t()
    i, j, depth, color, semantic = select_uv_semantic(i, j, n, depth, color, semantic, device=device)
    return i, j, depth, color, semantic

def get_samples(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2w, depth, color, device, return_ij = False):
    """
...
def get_samples_tracking_semantic(H0, H1, W0, W1, n, H, W, fx, fy, cx, cy, c2w, depth, color, semantic, device, return_ij = False):
    """
    Get n rays from the image region H0..H1, W0..W1.
    c2w is its camera pose and depth/color is the corresponding image tensor.

    """
    i, j, sample_depth, sample_color, sample_semantic = get_sample_uv_semantic(
        H0, H1, W0, W1, n, depth, color, semantic, device=device)
    rays_o, rays_d = get_rays_from_uv(i, j, c2w, H, W, fx, fy, cx, cy, device)
    
    if return_ij :
        return rays_o, rays_d, sample_depth, sample_color, sample_semantic, i, j
    else :
...
def raw2outputs_nerf_color(raw, z_vals, rays_d, occupancy=False, device='cuda:0'):
    ...
    def raw2alpha(raw, dists, act_fn=F.relu): 
        with torch.autograd.enable_grad():
            result = 1. - torch.exp(-act_fn(raw)*dists)
        return result
    
    dists = z_vals[..., 1:] - z_vals[..., :-1] # distance for sample points
    dists = dists.float()
    ...
    # 这里alpha.float()就不保留梯度了，可能有问题，试验一下看看
    # weights = alpha.float() * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)).to(
    #     device).float(), (1.-alpha + 1e-10).float()], -1).float(), -1)[:, :-1]
    weights = alpha.to(torch.float32) * torch.cumprod(torch.cat([torch.ones((alpha.shape[0], 1)).to(
        device).to(torch.float32), (1.-alpha + 1e-10).to(torch.float32)], -1).to(torch.float32), -1)[:, :-1]
...
def normalize_3d_coordinate_semantic(p, bound, voxel_shape):
    """
    Normalize coordinate to [-1, 1], corresponds to the bounding box given.
    ...
    """
    p = p.reshape(-1, 3)
    p[:, 0] = ((p[:, 0]-bound[0, 0])/(bound[0, 1]-bound[0, 0])) * voxel_shape[0]
    p[:, 1] = ((p[:, 1]-bound[1, 0])/(bound[1, 1]-bound[1, 0])) * voxel_shape[1]
    p[:, 2] = ((p[:, 2]-bound[2, 0])/(bound[2, 1]-bound[2, 0])) * voxel_shape[2]
    return p
